...
instantiate gbl h k (TyLambda x e) ty
  = (h,k,substTm ("Evaluator.instantiate on:" <> line <> "TyLambda"
                  <+> pretty x
                  <+> pretty e) subst e)
 where
  subst  = extendTvSubst subst0 x ty
  subst0 = mkSubst is0
  is0 = -- trace ("instantiate: gblVars = " ++ showDoc (pretty gblVars))
        mkInScopeSet (tyFVsOfTypes [ty]
                     `unionUniqSet` gblVars -- take the inscopeset from the global variables?
                     -- `unionUniqSet` fVsOfTerms [e] -- or from the free variables in e?
                     )
  gblVars :: VarSet
